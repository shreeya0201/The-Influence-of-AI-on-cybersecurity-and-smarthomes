...
def Predict_Cyber_Attack_Type(request):
    if request.method == "POST":
        ...
        mlp_model.add(Dense(3, activation='softmax'))  # Assuming 3 classes for your output
        mlp_model.compile(optimizer='adam', loss='sparse_categorical_crossentropy', metrics=['accuracy'])
        mlp_model.fit(X_train_scaled, y_train, epochs=50, batch_size=10)

        # Predict and evaluate the MLP classifier
        predict_mlp = np.argmax(mlp_model.predict(X_test_scaled), axis=-1)
        mlp_accuracy = accuracy_score(y_test, predict_mlp) * 100
        print("ANN")
        print("ACCURACY")
        print(mlp_accuracy)
